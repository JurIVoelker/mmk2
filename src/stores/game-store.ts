...
export const timeLimitText = 17;
export const timeLimitImage = 7;
        ...
        const enrichedData = await Promise.all(
          data.map(async (item: News) => {
            ...
            if (item.type === "video") {
              ...
              timeLimit =
                Math.min(
                  Math.max(duration, videoTimeConfig.MIN),
                  videoTimeConfig.MAX
                ) + videoTimeConfig.BUFFER;
            }
      ...
      classifyAsFakeNews: () => {
        set((state) => {
          ...
          return {
            ...
            timeLeft:
              state.unclassifiedNews[state.currentIndex + 1]?.timeLimit ??
              timeLimitDefault,
          };
        });
      },
